...
### 役（Formations）
#### 各フラグで3枚のカードから以下の役が成立します(強さ順)：
1. **ストレートフラッシュ（Straight Flush）**: 同じ色で連続した値
2. **スリーオブアカインド（3 of a Kind）**: 同じ値が3枚
3. **フラッシュ（Flush）**: 同じ色が3枚
4. **ストレート（Straight）**: 連続した値（色は異なる）
5. **役なし**:役ない

#### 各フラグで4枚のカードから以下の役が成立します(強さ順、泥カード使用時)：
1. **ストレートフラッシュ（Straight Flush）**: 同じ色で連続した値
2. **フォーオブアカインド（4 of a Kind）**: 同じ値が4枚
3. **フラッシュ（Flush）**: 同じ色が4枚
4. **ストレート（Straight）**: 連続した値（色は異なる）
5. **役なし（No Formation）**: 役が成立していない状態
...
#### 注意点
 - 特殊カードは、自分のターンで使用した際に、相手が特殊カードを使用するまで、特殊カードが利用できなくなります。
 - 自分のターンで特殊カードが利用できる際は、画面左側の黄色コインが表示されている場合のみ、特殊カードを使用可能です。
